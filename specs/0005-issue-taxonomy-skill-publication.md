...
| `CLAUDE.md:8` and `README.md:5` | *"THREE things live here and nothing else"* — enumerated the repository's **top-level structure** (decisions, conductor, install door), which this publication did not change. *(Both sentences were **removed** on 2026-08-04, unrelated to this publication: the enumeration had been false since `research/` landed on 2026-07-27 and the root holds eight directories. The scope rule it duplicated — "only genuinely cross-collective content lands here" — was always the operative one and is untouched.)* |
