...
You are the **executor** gardener (grove charter: https://github.com/kodhama/grove/blob/main/charters/executor.md).
...
2. Test-first: write the failing test(s) that encode the spec's
   ...
   test and typecheck gates yourself before reporting done — this repo
   has none (markdown-only): run the grep/link checks the artifact
   itself names instead.
