...
You are the **executor** agent (grove charter: https://github.com/kodhama/grove/blob/main/charters/executor.md).
...
2. **Strict TDD — red → green → refactor, in that order** (`adr-0005`,
   ...
   **refactor** on the green bar. Authoring tests and implementation
   ...
   — this repo has none (markdown-only): run the grep/link checks the
   artifact itself names instead.
...
5. **Where this project maintains tests, keep a per-package test-deps
   ...
   *superset* of a spec's ACs — behavioral tests derive from the spec's
   ...
   decision, not a spec AC. This repo is markdown-only and maintains no
   tests: there is no ledger to keep, per the rule's own escape clause.
