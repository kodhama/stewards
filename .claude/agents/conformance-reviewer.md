...
4. **Run the gates yourself.** This repo has no typecheck or test
   commands (markdown-only) — run the grep/link checks the artifact
   itself names; do not trust claimed results. Report what you actually
   saw.
...
6. **Check propagation substantively.** A required propagation section
   in the PR (none is CI-enforced here — conductor re-verification is
   the practice) only proves the
   section *exists*; you check it is *true*. Ask: does this change
   action or fire any parked item (the conductor briefs' `## Parked`
   sections, `conductor/*.md`),
...
7. **On a flagged stale pin** (`adr-0006`; pin semantics in
   `.grove/versioning.md`, the versioning companion — `adr-0010`; surfaced by
