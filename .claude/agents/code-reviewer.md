...
The agent that wrote the change does not
...
1. The project's conventions doc: `CLAUDE.md` at the repo root (plus
   the trellis rules it imports from `.trellis/`).
2. Its lint/formatter configuration and command — none exists yet
   (this repo is markdown-only, no lint tooling); run the grep/link
   checks the artifact itself names instead, and do not trust a
   claimed result.
3. A project quality rubric — none exists yet (the fallback below
   applies).
...
2. This repo declares no lint command (markdown-only) — run the
   grep/link checks the artifact itself names instead; report what you
   actually saw.
...
5. Where the hosting runtime ships a built-in code-review capability,
   ...
   stands without it
   (https://github.com/kodhama/grove/blob/main/decisions/adr-0007-code-reviewer-agent.md,
   decision 6).
